...
from scipy import dot,array,zeros,eye
from scipy.linalg import inv,expm,eig,pinv,norm
...
def skew_3d(omega):
    ...
    #YOUR CODE HERE
    omega_hat = np.matrix([[0, -omega[2], omega[1]],
    	[omega[2], 0, -omega[0]],
    	[-omega[1], omega[0], 0]])
...
def rotation_3d(omega, theta):
    ...
    rot = np.identity(3) + (omega_hat/omega_norm)*np.sin(theta*omega_norm) + (np.dot(omega_hat,omega_hat)/omega_norm**2)*(1-np.cos(theta*omega_norm)) 
...
def hat_3d(xi):
    ...
    #YOUR CODE HERE
    xi_hat = np.matrix([[0,-xi[5],xi[4],xi[0]],
    	[xi[5],0,-xi[3],xi[1]],
    	[-xi[4],xi[3],0,xi[2]],
    	[0,0,0,0]])   
...
def homog_2d(xi, theta):
    ...
    #YOUR CODE HERE
    omega = xi[2]
    R = np.matrix([[np.cos(theta*omega),-np.sin(theta*omega)],
    [np.sin(theta*omega),np.cos(theta*omega)]])

    p1 = np.matrix([[1-np.cos(theta*omega),np.sin(theta*omega)],
    [-np.sin(theta*omega),1-np.cos(theta*omega)]])

    p2 = np.matrix([[0,-1],[1,0]])
    p3 = np.divide(np.matrix([[xi[0]],[xi[1]]]),omega)

    p = np.matmul(np.matmul(p1,p2),p3)

    gtop = np.hstack((R,p))
    g = np.vstack((gtop,np.array([0,0,1])))
    print("g")
    print(g)
